...
export class BiliManage extends plugin {
  constructor() {
    super({
      ...
      rule: [
        ...
        { reg: '^#bili\\s*(过滤列表|fl)\\s*(.*)$', fnc: 'filterList' },
        { reg: '^#bili\\s*(过滤删除|fd)\\s*(.*)$', fnc: 'filterDel' },
      ],
    })
  }
  ...
  /** #bili颜色 <UID/用户名> <#hex> */
  async color(e) {
    const args = splitArgs(e.msg.replace(/^#bili\s*(颜色|主题色|color)\s*/, ''))
    if (args.length < 2) return e.reply('用法：#bili颜色/color <UID/用户名> <#颜色>（多色用分号分隔，如 #fde8ed;#d3edfa）')
  ...
  /** #bili模板 <d/l/c> <模板名> */
  async template(e) {
    const args = splitArgs(e.msg.replace(/^#bili\s*(模板|推送模板|t)\s*/, ''))
    if (args.length < 2 || !/^(d|l|c|le)$/.test(args[0])) {
      return e.reply('用法：#bili模板/t <d/l/c/le> <模板名>\nd=动态推送 l=直播推送 c=直播结束(le)')
    }
    const kind = args[0]
  ...
  /** #bili模板列表 [d/l/c] */
  templateList(e) {
    const kind = (e.msg.replace(/^#bili\s*(模板列表|模板效果|tl)\s*/, '').trim() || 'd').toLowerCase()
    if (!/^(d|l|c|le)$/.test(kind)) return e.reply('模板类型请使用 d(动态) / l(直播) / c 或 le(直播结束)')
    const names =
      kind === 'd'
  ...
  /** #biliat全体 <类型> [UID] [群号] */
  async atAll(e) {
    const args = splitArgs(e.msg.replace(/^#bili\s*(at全体|atall|aa)\s*/, ''))
  ...
  async delAtAll(e) {
    const args = splitArgs(e.msg.replace(/^#bili\s*(取消at全体|delAtall|daa)\s*/, ''))
  ...
  /** #biliat全体列表 [UID] [群号] */
  async listAtAll(e) {
    const args = splitArgs(e.msg.replace(/^#bili\s*(at全体列表|atall列表|laa)\s*/, ''))
    const uid = parseUidArg(args[0] ?? '')
    if (uid === null) return e.reply('未匹配到用户哦')
    if (uid === undefined) return e.reply('匹配到多个用户，请使用 UID 重试')
  ...
  async filterType(e) {
    ...
    const args = splitArgs(e.msg.replace(/^#bili\s*(类型过滤|ft)\s*/, ''))
  ...
  async filterReg(e) {
    if (!canManage(e)) return e.reply('仅群管理员或主人可以设置过滤器')
    const args = splitArgs(e.msg.replace(/^#bili\s*(正则过滤|fr)\s*/, ''))
  ...
  async filterDel(e) {
    if (!canManage(e)) return e.reply('仅群管理员或主人可以设置过滤器')
    const args = splitArgs(e.msg.replace(/^#bili\s*(过滤删除|fd)\s*/, ''))
